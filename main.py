...
async def price(ctx, arg, arg2='USD'):  # arg is crypto, arg2 is the currency
    ...
    embed.add_field(name="Price:", value="```" + "$" + str(pricesearch[str(arg.lower())][str(arg2.lower())]) + "```", inline=False)
    embed.add_field(name="24H Change:", value="```" + str(pricesearch[str(arg.lower())][str(arg2.lower()) + "_24h_change"]) + "```",
                    inline=False)
